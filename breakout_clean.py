...
class BreakoutTrader:
    """Breakout MIT ladder trading strategy."""

    def __init__(self, account_id, token, instrument="EUR_USD", units=1000, pip_size=0.0001):
    ...
    def place_ladder_orders(self, bid, ask, m, n, wait=0.5):
        for i in range(m, n + 1):
            ...
            if order_id:
                self.pending_trades[round(price, 5)] = order_id
                if direction == "BUY":
                    self.buy_orders.append(round(price, 5))
                else:
                    self.sell_orders.append(round(price, 5))
            time.sleep(wait)
    ...
    def log_fill(self, side, price, order_id, fill_price, log_file="fills.csv"):
        with open(log_file, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([datetime.now().isoformat(), side, price, order_id, fill_price])

    def export_recent_trades(self, minutes=5, filename="closed_trades.csv"):
        try:
